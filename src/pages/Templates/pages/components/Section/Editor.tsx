...
export const Editor = ({ content, item, PlaceholderService }: Props) => {
  ...
  const setPlaceholder = usePlaceholderUpdateModal(
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      if (!e.ctrlKey) return;
      const isSpanPlaceholder = (e.target as HTMLElement).getAttribute(
        "data-template-it_id"
      );
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    // TODO body.innerHTML
    const response = await PlaceholderService.update({
      id: item.id,
      content: body.innerHTML,
      title: item.title,
      placeholders: placeholders,
    });
    const parsed = handleResponse<Component | Section>(
      response,
      location,
      navigate
    );
  ...
  const handleAddPlaceholder = () => {
    ...
    span.style.cssText =
      "cursor: pointer; padding: 0.2rem 0.4rem; border-radius: 0.2rem; border: 1px solid black; background: #ececec; font-size: 14px; box-shadow: 0px 0px 5px #00000060";
            ...
            <div className="p-4 bg-slate-50 rounded space-y-2">
              <Label className="flex flex-col gap-2">
                ...
                <div className="flex items-stretch gap-2">
                  ...
                  />
                  <Button variant={"outline"} onClick={handleAddPlaceholder}>
