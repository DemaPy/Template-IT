...
const Section = ({ item }: Props) => {
  ...
  const {
    ...
    error: deletingError,
  } = useDeleteSection({ invalidate_key: item.templateId });
  const {
    mutate: duplicate,
    isPending: isDuplicating,
    isError: isDuplicatingError,
    error: duplicatingError,
  } = useDuplicate({ invalidate_key: item.templateId });
  ...
  if (isDeletingError) {
    return (
      <ErrorPage
        ...
        path={`/templates/${item.templateId}`}
      />
    );
  }
  ...
  if (isDuplicatingError) {
    return (
      <ErrorPage
        error={duplicatingError}
        message={duplicatingError.message}
        path={`/templates/${item.templateId}`}
      />
    );
  }
  ...
  return (
    <li className="w-full flex flex-col gap-4 border rounded-md p-2 bg-white">
      ...
      />
      {isOpen && <Placeholders placeholders={item.placeholders} />}
    </li>
  );
};
