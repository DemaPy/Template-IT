...
function Register() {
    ...
    const { formState: { errors }, setError, register, handleSubmit } = useForm<FormValues>(options);
    ...
    return (
        <div className="flex items-center justify-center mx-10 h-full">
            <Card className="min-w-96 max-w-xl">
                ...
                <form onSubmit={handleSubmit(({ email, submit_email, password }) => {
                    if (email !== submit_email) {
                        setError("submit_email", { message: "Emails should be the same" }, {
                            shouldFocus: true
                        })
                    ...
                    <CardFooter className="gap-2">
                        <Button disabled={isPending || isSuccess} className="w-full" type="submit">Register</Button>
                    </CardFooter>
                </form>
            </Card>
            <Toaster />
