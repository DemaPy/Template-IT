...
const ConnectDataWithPlaceholder = ({
  ...
  const onSubmit = async () => {
    ...
    if (!Object.keys(result).length) {
      ...
      setIsLoading(true);
      const response = await CampaignServiceDB.savePlaceholderData({
            ...
            {section?.placeholders.map((item, idx) => {
              return (
                <div className="grid grid-cols-6 gap-2" key={idx}>
                  <p className="text-sm font-semibold border p-2 rounded-md col-span-4">
                    {item.title}
                  </p>
                  <Select
                    value={selectedColumns[item.id] || ""}
                    onValueChange={(value) =>
                      handleSelectColumn({ placeholder_id: item.id, value })
                    }
                  >
