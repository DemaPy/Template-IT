...
const SectionLayout = ({ isLayoutChanged, section, moveCard, id, sectionId, index, is_active, order, renderOn }: Props) => {
  ...
  >({
    ...
    const handleLayoutIsActive = async () => {
        setIsLoading(true)
        const response = await CampaignService.updateLayout({ id: id, is_active: !is_active })
    ...
    return (
        <>
            ...
            >
                ...
                <Switchh isDisabled={isLoading || isLayoutChanged} text={is_active ? "On" : "Off"} isActive={is_active} onChange={() => handleLayoutIsActive()} />

            </div>
            {isOpen && renderOn && <SectionSlugs isLoading={isLoading} onChange={handleSlugRenderConditionChange} slugs={renderOn} />}
        </>
    )
}
